...
#+NAME: gps 
#+BEGIN_SRC lisp :noweb tangle :tangle gps-v-three.lisp
  ;; This file was auto generated from
  ;; general-problem-solver.org
...
#+END_SRC
* VERSION FOUR
The blocks world shows that GPS still clobbers sibling goals. And it tries inefficient solutions.
...
#+NAME: achieve-each-v-four
#+BEGIN_SRC lisp
  (defun achieve-each (state goals goal-stack)
    "Achieve each goal, and make sure they hold at the end."
    ;; don't mutate the global state
    (let ((current-state state))
      (if (and
           ;; mutuate the current state
           ;; by mapping achieve onto each goal
           ;; and seeing if all the goals are achievable
           (every #'(lambda (g) (setf current-state
                                      (achieve current-state g goal-stack)))
                  goals)
           ;; if the goals are all achievable
           ;; then test that the mutated current-state
           ;; to see if any are clobbered
           (subsetp goals current-state :test #'equal))
          ;; if none are clobbered return the mutated current state
          current-state)))
#+END_SRC
** orderings
#+NAME: orderings-v-four
#+BEGIN_SRC lisp
  (defun orderings (l)
    (if (> (length l) 1)
        (list l (reverse l))
...
#+BEGIN_SRC lisp
  (defun achieve (state goal goal-stack)
    "A goal is achieved when it already holds or if there is an appropriate op that is applicable."
    (debug-indent :gps (length goal-stack) "Goal: ~a" goal)
    (cond ((member-equal goal state) state)
          ((member-equal goal goal-stack) nil)
          (t (some #'(lambda (op)
                       (apply-op state goal op goal-stack))
...
#+END_SRC
** appropriate-ops
At this point we've got lambdas inside of lambdas and that's often a code smell. I mean it's lambdas all the way down, but at some point it looks like a rabbit hole.
#+NAME: appropriate-ops-v-four
#+BEGIN_SRC lisp :noweb tangle
  (defun appropriate-ops (goal state)
    "Return a list of appropriate operators sorted by the number of unfilled preconditions."
    (sort (copy-list (find-all goal *ops* :test #'appropriate-p))
          #'<
          :key <<appropriate-ops-key>>))
#+END_SRC
The key for sorting returns a number that represents how many preconditions an =op= has that are not in the state.
...
#+END_SRC
** GPSv4 Block Ops
#+BEGIN_SRC lisp :tangle gps-v4-block-ops.lisp :noweb tangle
  ;;; This file AUTOGENERATED
